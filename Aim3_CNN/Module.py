...
def Feature_Clustering(Embedding_data,args):
    ...
    #if args.n_components == 3:
    #    model.load_state_dict(torch.load('data/weight_ours.pth'))
    ...
    #loss_hpy = torch.nn.HuberLoss(reduction='mean', delta=2)
    #loss_hpz = torch.nn.HuberLoss(reduction='mean', delta=2)
    ...
    torch.save(model.state_dict(), 'data/weight_ours.pth')
    ...
    for epoch in range(args.maxIter):
        ...
        if args.use_scribble == True:
            ...
            loss = args.stepsize_sim * loss_sim(output[inds_sim],target[inds_sim]) + \
                   ...
                   args.stepsize_con * (lhpy + lhpz) + args.stepsize_ent *loss_ent(output)
    ...
    if args.visualize:
        ...
        #cv2.imshow("output", im_target_rgb)
        cv2.waitKey(50)
        cv2.imwrite(args.output_file + '%.1f_%.1f_%.1f.png'% (args.stepsize_sim,args.stepsize_con,args.stepsize_ent),im_target_rgb)
